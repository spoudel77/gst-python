...
def colToProtRep(filename, output1, output2, pepNo, specNo):
	d1_pep = {}
	d2_spec = {}
	d3_moff = {}
	filProtLst = []
...
for y in pepReports:
	...
	filteredProt=colToProtRep(y, new_name, new_name2, int(pepCnt), int(specCnt))
...
unfilt_protReports = glob.glob(cruxDir+'/'+'*_final_sample_unfiltered_prot_report.txt')

for unprotFile in unfilt_protReports:
	unnormProt = unprotFile.split('_final_sample_unfiltered_prot_report.txt')[0]+'_final_unnormalized_prot_report.txt'
	...
	unpepSum = 0
	unspecSum = 0
	with open(unprotFile,'r') as unprotDict:
		for unp_line in unprotDict:
			if 'maxIntensity' not in unp_line:
				untp_line = unp_line.split('\t')
				unmoff_Int = float(untp_line[-1])
				unpepSum = int(untp_line[2])
				unspecSum = int(untp_line[3])
				uneachProtDict[untp_line[1]]=unmoff_Int
				uneachPepDict[untp_line[1]]=str(unpepSum)+'\t'+str(unspecSum)
	
	with open(unnormProt,'w') as unnormFile:
		unnormFile.write('file_id_protein\ttotPepCnt\ttotSpecCnt\tIntensity\n')
		for uneach_key in uneachProtDict.keys():
			# normInt = eachProtDict[each_key]/ML_Sum
			
			unnormFile.write(uneach_key+'\t'+uneachPepDict[uneach_key]+'\t'+str(uneachProtDict[uneach_key])+'\n')


unnormProtFile = glob.glob(cruxDir+'/'+'*_final_unnormalized_prot_report.txt')
unprot_List = filesToList(unnormProtFile)
...
def file_to_final_files(file_final_ext, ddd, pep_dd, spec_dd):
...
file_to_final_files('normalized_final_output.txt', dd1, pep_dct1, spec_dct1)
file_to_final_files('unnormalized_final_output.txt', dd2, pep_dct2, spec_dct2)
