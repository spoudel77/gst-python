...
# print header
os.chdir(cruxDir)
with open('final_peptide_matrix_unfiltered.txt','a') as g:
...
pep_matrix = "final_peptide_matrix_unfiltered.txt"
prot_matrix = "normalized_final_output.txt"

df1 = pd.read_csv(pep_matrix,delimiter='\t')
df3 = pd.read_csv(prot_matrix,delimiter='\t')


epi=df1.loc[df1['protein'].isin(df3['protein'])]
epi.to_csv("final_pep_matrix_filter.csv")
