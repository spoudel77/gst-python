...
if len(mzml_lst)!=0:
	...
	for fasta_file in fasta_list:
		code_Decoy = output1+'/removeNewLIneDecoyDatabase.py'
		new_Fasta_in = fasta_file.split('/')[-1].split('.fasta')
		new_Fasta_file = new_Fasta_in[0]+'_FR.fasta'
		Index = 'Index_'+new_Fasta_in[0]
		IndexFile = 'IndexFile_'+new_Fasta_in[0]
		...
		subprocess.call([crux,'tide-index','--output-dir',Index,'--decoy-format','protein-reverse','--mods-spec',mod,'--clip-nterm-methionine','T',fasta_file,IndexFile])

	#samples = fileToSample(mzml_lst)
	tide_input = ' '.join(mzml_lst)
	#crux = '/home/pue/crux-3.1.Linux.x86_64/bin/crux'
	test =[crux,'tide-search','--compute-sp','T','--concat','T','--precursor-window','10','--precursor-window-type','ppm','--output-dir','ConcatPerc']
	...
	subprocess.call([crux,'percolator','--test-fdr','0.01','--train-fdr','0.01','--only-psms','T','--protein-report-duplicates','T','--protein-report-fragments','T','--search-input','concatenated','--overwrite','T','--output-dir',perc_folder,concat_tide])
